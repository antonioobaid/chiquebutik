      ...
      <div className="max-w-6xl w-full bg-white/90 dark:bg-gray-800/90 backdrop-blur-md rounded-3xl shadow-xl p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 gap-10 relative">
        ...
        <div className="space-y-4">
          <div className="relative rounded-2xl overflow-hidden bg-gray-100 dark:bg-gray-700 aspect-square">
            ...
            {images.length > 1 && (
              <>
                <button onClick={prevImage} className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-700 rounded-full p-2 shadow-lg transition-all z-10">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                  </svg>
                </button>
                <button onClick={nextImage} className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-700 rounded-full p-2 shadow-lg transition-all z-10">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col justify-between">
          <div>
            <h1 className={`text-4xl sm:text-5xl font-extrabold mb-4 ${isProductSoldOut ? 'text-gray-500' : 'text-gray-900 dark:text-white'}`}>
              {product.title}
            </h1>
            <p className={`mb-6 ${isProductSoldOut ? 'text-gray-400' : 'text-gray-700 dark:text-gray-300'}`}>
              {product.description}
            </p>
            <p className={`text-2xl font-bold mb-4 ${isProductSoldOut ? 'text-gray-400 line-through' : 'text-blue-500'}`}>
              {product.price} kr
            </p>
